...
FOOT;
    ...
    public static function database(bool $reader): PDO
    {
        ...
        try {
            ...
            ThrowableCatcher::catchThrowable($e, true);
    ...
    public static function TryCatchPDOException(PDOException $e): void
    {
        ...
        $error_array = ThrowableCatcher::catchThrowable($e, true);

        $log_array = $error_array[ThrowableCatcher::LOG_ARRAY];

        // todo - handle all pdo exceptions
        switch ((string)$e->getCode()) {        // Database has not been created
            case '0':

                print ThrowableCatcher::generateBrowserReport($log_array);  // this terminates
            ...
            case '1049':
                ...
                try {
                    ...
                    $error_array_two = ThrowableCatcher::catchThrowable($e);
                    ...
                    ThrowableCatcher::generateBrowserReport($error_array_two);  // this terminates
            ...
            case '42S02':
                ...
                print ThrowableCatcher::generateBrowserReport($log_array);
    ...
    protected static function newInstance(bool $reader = false): PDO
    {
        ...
        do {
            ...
            try {
                ...
                ThrowableCatcher::catchThrowable($e);  // this will exit
    ...
    public static function verifyAndCreateForeignKeyRelations(string $fullyQualifiedClassName, callable $cb): bool
    {
        ...
        foreach ($fullyQualifiedClassName::INTERNAL_TABLE_CONSTRAINTS as $internalTableColumn => $externalTableColumn) {
            ...
            if ([] === $values) {
                ...
                ColorCode::colorCode("Failed to verify that the table ($internalTableName) contains FOREIGN KEY CONSTRAINT ($externalTableName.$externalColumnName) => ($internalTableName.$internalColumnName)", iColorCode::RED);
                ...
                if ('' === $constraintName) {
                    ...
                    ColorCode::colorCode("Failed to find the constraint name in the table creation SQL \n($tableCreateSTMT)\n with regex ($constraintNameRegex)\n\n", iColorCode::RED);
                ...
                ColorCode::colorCode("Attmpting to run :: ($addConstraint)", iColorCode::CYAN);

                if (false === self::execute($addConstraint)) {

                    ColorCode::colorCode("Failed to create constraint using generated sql ::", iColorCode::RED);

                    ColorCode::colorCode("$addConstraint\n\n", iColorCode::RED);
                ...
                ColorCode::colorCode('The missing constraint was automatically added. You should re-run the build.');
            ...
            ColorCode::colorCode("Verified relation $internalTableName.$internalColumnName => $externalTableName.$externalColumnName", iColorCode::BACKGROUND_MAGENTA);
    ...
    public static function scanAndRunRefreshDatabase(string $tableDirectory): bool
    {
        ...
        foreach (self::$tablesToValidateAfterRefresh as $fullyQualifiedClassName => $preUpdateSQL) {
            ...
            if (false === self::verifyAndCreateForeignKeyRelations($fullyQualifiedClassName, $pregMatchSchema)) {
                ...
                ColorCode::colorCode("Failed druing verifyAndCreateForeignKeyRelations:", iColorCode::RED);
            ...
            $awsLoose = Interfaces\iRest::SQL_VERSION_PREG_REPLACE;
            ...
            if ([] !== $changesOne || $changesTwo !== []) {
                ...
                ColorCode::colorCode("Note: Should the changes below in cyan be out-of-date, you need only rebuild the autogenerated rest classes.", iColorCode::MAGENTA);
    ...
    public static function columnExistsOrExecuteSQL(string $column, string $table_name, string $sql): void
    {
        ...
        if ([] === $existed) {
            ...
            ColorCode::colorCode("Column ($column) did not appear to exist. Attempting to run ($sql).");
            ...
            ColorCode::colorCode("The ($column) already exists on table ($table_name).");
    ...
    public static function addTablePrefix(string &$table_name, string $table_prefix, string &$sql): void
    {
        ...
        if (str_contains($sqlReplaced, "`$table_name`")
