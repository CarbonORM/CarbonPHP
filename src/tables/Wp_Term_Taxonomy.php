...
class Wp_Term_Taxonomy extends Rest implements iRestSinglePrimaryKey
{
    ...
    /**
     * COLUMNS
     * The columns below are a 1=1 mapping to your columns found in wp_term_taxonomy. Remember to regenerate when as all changes  
    ...
    */
    ...
    {   
        ...
        if (!self::validateInternalColumn(self::POST, $ref, $op, $taxonomy, $taxonomy === '')) {
            throw new PublicAlert('Your custom restful api validations caused the request to fail on column \'wp_term_taxonomy.taxonomy\'.');
        }
        $stmt->bindParam(':taxonomy',$taxonomy, PDO::PARAM_STR, 32);
        
        if (!array_key_exists('wp_term_taxonomy.description', $data)) {
            throw new PublicAlert('The column \'wp_term_taxonomy.description\' is set to not null and has no default value. It must exist in the request and was not found in the one sent.');
        } 
        $ref='wp_term_taxonomy.description';
        $op = self::EQUAL;
        if (!self::validateInternalColumn(self::POST, $ref, $op, $data['description'])) {
            throw new PublicAlert('Your custom restful api validations caused the request to fail on column \'wp_term_taxonomy.description\'.');
        }
        ...
        if (self::$commit && !Database::commit()) {
           throw new PublicAlert('Failed to store commit transaction on table wp_term_taxonomy');
        }
    ...
    public static function Put(array &$returnUpdated, string $primary, array $argv) : bool
    {
        ...
        foreach ($argv as $key => &$value) {
            ...
            if (!self::validateInternalColumn(self::PUT, $key, $op, $value)) {
                throw new PublicAlert('Your custom restful api validations caused the request to fail on column \'wp_term_taxonomy.\'.');
            }
        }
        unset($value);
