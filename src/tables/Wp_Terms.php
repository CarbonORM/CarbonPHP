 ...
 *  static method.
 */
class Wp_Terms extends Rest implements iRestSinglePrimaryKey
{
    use RestfulValidations;
    
    public const CLASS_NAME = 'Wp_Terms';
    public const CLASS_NAMESPACE = 'CarbonPHP\Tables\\';
    public const TABLE_NAME = 'wp_terms';
    public const TABLE_PREFIX = '';
    
    /**
     * COLUMNS
     * The columns below are a 1=1 mapping to the columns found in wp_terms. 
    ...
    **/
    public const TERM_ID = 'wp_terms.term_id'; 

    public const NAME = 'wp_terms.name'; 

    public const SLUG = 'wp_terms.slug'; 

    public const TERM_GROUP = 'wp_terms.term_group'; 
    ...
    **/
    public const PRIMARY = 'wp_terms.term_id';
    ...
    **/ 
    public const COLUMNS = [
        'wp_terms.term_id' => 'term_id','wp_terms.name' => 'name','wp_terms.slug' => 'slug','wp_terms.term_group' => 'term_group',
    ];
...
MYSQL;
    ...
    */
    ...
    {   
        ...
        $name = $data['wp_terms.name'] ?? '';
        $ref='wp_terms.name';
        $op = self::EQUAL;
        if (!self::validateInternalColumn(self::POST, $ref, $op, $name, $name === '')) {
            return self::signalError('Your custom restful api validations caused the request to fail on column \'wp_terms.name\'.');
        }
        $stmt->bindParam(':name',$name, PDO::PARAM_STR, 200);
        
        $slug = $data['wp_terms.slug'] ?? '';
        $ref='wp_terms.slug';
        $op = self::EQUAL;
        if (!self::validateInternalColumn(self::POST, $ref, $op, $slug, $slug === '')) {
            return self::signalError('Your custom restful api validations caused the request to fail on column \'wp_terms.slug\'.');
        }
        $stmt->bindParam(':slug',$slug, PDO::PARAM_STR, 200);
                 
        $term_group = $data['wp_terms.term_group'] ?? '0';
        $ref='wp_terms.term_group';
        $op = self::EQUAL;
        if (!self::validateInternalColumn(self::POST, $ref, $op, $term_group, $term_group === '0')) {
            return self::signalError('Your custom restful api validations caused the request to fail on column \'wp_terms.term_group\'.');
        }
        ...
        if (self::$commit && !Database::commit()) {
            return self::signalError('Failed to store commit transaction on table wp_terms');
        }
    ...
    public static function Put(array &$returnUpdated, string $primary, array $argv) : bool
    {
        ...
        foreach ($argv as $key => &$value) {
            ...
            if (!self::validateInternalColumn(self::PUT, $key, $op, $value)) {
                return self::signalError('Your custom restful api validations caused the request to fail on column \'wp_terms.\'.');
            }
        }
        unset($value);

        $sql = /** @lang MySQLFragment */ 'UPDATE wp_terms SET '; // intellij cant handle this otherwise
        ...
        if (array_key_exists('wp_terms.slug', $argv)) {
            $set .= 'slug=:slug,';
        }
        if (array_key_exists('wp_terms.term_group', $argv)) {
            $set .= 'term_group=:term_group,';
        }
        ...
        if (array_key_exists('wp_terms.term_id', $argv)) {
            ...
            $slug = $argv['wp_terms.slug'];
            $ref = 'wp_terms.slug';
            $op = self::EQUAL;
            if (!self::validateInternalColumn(self::PUT, $ref, $op, $slug)) {
                return self::signalError('Your custom restful api validations caused the request to fail on column \'slug\'.');
            }
            $stmt->bindParam(':slug',$slug, PDO::PARAM_STR, 200);
        }if (array_key_exists('wp_terms.term_group', $argv)) {
        ...
        $argv = array_combine(
            array_map(
                static function($k) { return str_replace('wp_terms.', '', $k); },
                array_keys($argv)
            ),
            array_values($argv)
        );
    ...
    public static function Delete(array &$remove, string $primary = null, array $argv = []) : bool
    {
        ...
        if (null === $primary) {
            ...
            */
            ...
            $sql .= ' WHERE  term_id='.self::addInjection($primary, $pdo).'';
        }
