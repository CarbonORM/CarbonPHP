...
class Carbon_Features extends Rest implements iRest
{
    ...
    */
    ...
    public static function Post(array $argv, string $dependantEntityId = null)
    {
        ...
        $feature_entity_id = $id = $argv['carbon_features.feature_entity_id'] ?? self::beginTransaction('carbon_features', $dependantEntityId);
    ...
    public static function validateSelectColumn($column) : bool {
        return (bool) preg_match('#(((((hex|argv|count|sum|min|max) *\(+ *)+)|(distinct|\*|\+|-|/| |carbon_features||\.feature_entity_id|\.feature_code|\.feature_creation_date))+\)*)+ *(as [a-z]+)?#i', $column);
    }
    ...
    public static function buildSelectQuery(string $primary = null, array $argv, PDO $pdo = null, bool $noHEX = false) : string 
    {
        ...
        if (array_key_exists(self::PAGINATION,$argv)) {
            ...
            if (!empty($limit)) {
                ...
                if (array_key_exists(self::ORDER,$argv[self::PAGINATION]) && is_string($argv[self::PAGINATION][self::ORDER])) {
                    ...
                    $order .= 'feature_entity_id ASC';
                }
            }
            $limit = "$order $limit";
        } else {
            $limit = ' ORDER BY feature_entity_id ASC LIMIT 100';
        }
        ...
        $sql = 'SELECT ' .  $sql . ' FROM carbon_features ' . $join;
       
        if (null === $primary) {
            /** @noinspection NestedPositiveIfStatementsInspection */
            if (!empty($where)) {
                $sql .= ' WHERE ' . self::buildWhere($where, $pdo, 'carbon_features', self::PDO_VALIDATION);
            }
        } else {
            $sql .= ' WHERE  feature_entity_id=UNHEX('.self::addInjection($primary, $pdo, 'carbon_features').')';
        }
