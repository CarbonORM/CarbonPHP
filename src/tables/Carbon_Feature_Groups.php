...
class Carbon_Feature_Groups extends Rest implements iRestfulReferences
{
    
    public const TABLE_NAME = 'carbon_feature_groups';
    public const GROUP_NAME = 'carbon_feature_groups.group_name'; 
    public const ENTITY_ID = 'carbon_feature_groups.entity_id'; 
    public const CREATED_BY = 'carbon_feature_groups.created_by'; 
    public const CREATION_DATE = 'carbon_feature_groups.creation_date'; 
    ...
    public const PDO_VALIDATION = [
        'carbon_feature_groups.group_name' => ['varchar', '2', '20'],'carbon_feature_groups.entity_id' => ['binary', '2', '16'],'carbon_feature_groups.created_by' => ['binary', '2', '16'],'carbon_feature_groups.creation_date' => ['datetime', '2', ''],
    ];
    ...
    */
    public static function Get(array &$return, array $argv): bool
    {
        ...
        if (!$stmt->execute()) {
            throw new PublicAlert('Failed to execute the query on Carbon_Feature_Groups.');
        }
    ...
    public static function Post(array $argv, string $dependantEntityId = null): bool
    {
        ...
        $sql = 'INSERT INTO carbon_feature_groups (group_name, entity_id, created_by) VALUES ( :group_name, UNHEX(:entity_id), UNHEX(:created_by))';
        ...
        $group_name = $argv['carbon_feature_groups.group_name'];
        $stmt->bindParam(':group_name',$group_name, 2, 20);
    
        $entity_id =  $argv['carbon_feature_groups.entity_id'] ?? null;
        $stmt->bindParam(':entity_id',$entity_id, 2, 16);
    
        $created_by =  $argv['carbon_feature_groups.created_by'] ?? null;
    ...
    public static function validateSelectColumn($column) : bool {
        return (bool) preg_match('#(((((hex|argv|count|sum|min|max) *\(+ *)+)|(distinct|\*|\+|-|/| |carbon_feature_groups||\.group_name|\.entity_id|\.created_by|\.creation_date))+\)*)+ *(as [a-z]+)?#i', $column);
    }
    ...
    public static function buildSelectQuery(string $primary = null, array $argv, PDO $pdo = null, bool $noHEX = false) : string 
    {
        ...
        $sql = 'SELECT ' .  $sql . ' FROM carbon_feature_groups ' . $join;
       
        if (null === $primary) {
            /** @noinspection NestedPositiveIfStatementsInspection */
            if (!empty($where)) {
                $sql .= ' WHERE ' . self::buildWhere($where, $pdo, 'carbon_feature_groups', self::PDO_VALIDATION);
            }
        } 
    ...
    */
    public static function Put(array &$return,  array $argv) : bool
    {
        ...
        $sql = 'UPDATE carbon_feature_groups ' . ' SET '; // intellij cant handle this otherwise

        $set = '';

        if (array_key_exists('carbon_feature_groups.group_name', $argv)) {
            $set .= 'group_name=:group_name,';
        }
        if (array_key_exists('carbon_feature_groups.entity_id', $argv)) {
            $set .= 'entity_id=UNHEX(:entity_id),';
        }
        if (array_key_exists('carbon_feature_groups.created_by', $argv)) {
            $set .= 'created_by=UNHEX(:created_by),';
        }
        if (array_key_exists('carbon_feature_groups.creation_date', $argv)) {
        ...
        if (array_key_exists('carbon_feature_groups.entity_id', $argv)) {
            $entity_id = $argv['carbon_feature_groups.entity_id'];
            $stmt->bindParam(':entity_id',$entity_id, 2, 16);
        }
        if (array_key_exists('carbon_feature_groups.created_by', $argv)) {
            $created_by = $argv['carbon_feature_groups.created_by'];
            $stmt->bindParam(':created_by',$created_by, 2, 16);
        }
        if (array_key_exists('carbon_feature_groups.creation_date', $argv)) {
            $stmt->bindValue(':creation_date',$argv['carbon_feature_groups.creation_date'], 2);
        }

        self::bind($stmt);

        if (!$stmt->execute()) {
            throw new PublicAlert('Restful table Carbon_Feature_Groups failed to execute the update query.');
        }
        
        $argv = array_combine(
            array_map(
                static function($k) { return str_replace('carbon_feature_groups.', '', $k); },
                array_keys($argv)
            ),
            array_values($argv)
        );
    ...
    */
    public static function Delete(array &$remove, array $argv) : bool
    {
        ...
        $sql = 'DELETE FROM carbon_feature_groups ';

        $pdo = self::database();
        
               
        $sql .= ' WHERE ' . self::buildWhere($argv, $pdo, 'carbon_feature_groups', self::PDO_VALIDATION);
