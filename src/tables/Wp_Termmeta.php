...
class Wp_Termmeta extends Rest implements iRestSinglePrimaryKey
{
    ...
    /**
     * COLUMNS
     * The columns below are a 1=1 mapping to your columns found in wp_termmeta. Remember to regenerate when as all changes  
    ...
    */
    ...
    {   
        ...
        if (!self::validateInternalColumn(self::POST, $ref, $op, $term_id, $term_id === '0')) {
            throw new PublicAlert('Your custom restful api validations caused the request to fail on column \'wp_termmeta.term_id\'.');
        }
        ...
        if (!self::validateInternalColumn(self::POST, $ref, $op, $meta_key, $meta_key === null)) {
            throw new PublicAlert('Your custom restful api validations caused the request to fail on column \'wp_termmeta.meta_key\'.');
        }
        $stmt->bindParam(':meta_key',$meta_key, PDO::PARAM_STR, 255);
        
        if (!array_key_exists('wp_termmeta.meta_value', $data)) {
            throw new PublicAlert('The column \'wp_termmeta.meta_value\' is set to not null and has no default value. It must exist in the request and was not found in the one sent.');
        } 
        $ref='wp_termmeta.meta_value';
        $op = self::EQUAL;
        if (!self::validateInternalColumn(self::POST, $ref, $op, $data['meta_value'])) {
            throw new PublicAlert('Your custom restful api validations caused the request to fail on column \'wp_termmeta.meta_value\'.');
        }
        ...
        if (self::$commit && !Database::commit()) {
           throw new PublicAlert('Failed to store commit transaction on table wp_termmeta');
        }
    ...
    public static function Put(array &$returnUpdated, string $primary, array $argv) : bool
    {
        ...
        foreach ($argv as $key => &$value) {
            ...
            if (!self::validateInternalColumn(self::PUT, $key, $op, $value)) {
                throw new PublicAlert('Your custom restful api validations caused the request to fail on column \'wp_termmeta.\'.');
            }
        }
        unset($value);
        ...
        $argv = array_combine(
            array_map(
                static function($k) { return str_replace('wp_termmeta.', '', $k); },
                array_keys($argv)
            ),
            array_values($argv)
        );
