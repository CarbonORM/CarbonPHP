...
class Wp_Options extends Rest implements iRestSinglePrimaryKey
{
    ...
    /**
     * COLUMNS
     * The columns below are a 1=1 mapping to your columns found in wp_options. Remember to regenerate when as all changes  
    ...
    */
    ...
    {   
        ...
        if (!self::validateInternalColumn(self::POST, $ref, $op, $option_name, $option_name === '')) {
            throw new PublicAlert('Your custom restful api validations caused the request to fail on column \'wp_options.option_name\'.');
        }
        $stmt->bindParam(':option_name',$option_name, PDO::PARAM_STR, 191);
        
        if (!array_key_exists('wp_options.option_value', $data)) {
            throw new PublicAlert('The column \'wp_options.option_value\' is set to not null and has no default value. It must exist in the request and was not found in the one sent.');
        } 
        $ref='wp_options.option_value';
        $op = self::EQUAL;
        if (!self::validateInternalColumn(self::POST, $ref, $op, $data['option_value'])) {
            throw new PublicAlert('Your custom restful api validations caused the request to fail on column \'wp_options.option_value\'.');
        }
        ...
        if (!self::validateInternalColumn(self::POST, $ref, $op, $autoload, $autoload === 'yes')) {
            throw new PublicAlert('Your custom restful api validations caused the request to fail on column \'wp_options.autoload\'.');
        }
        ...
        if (self::$commit && !Database::commit()) {
           throw new PublicAlert('Failed to store commit transaction on table wp_options');
        }
    ...
    public static function Put(array &$returnUpdated, string $primary, array $argv) : bool
    {
        ...
        foreach ($argv as $key => &$value) {
            ...
            if (!self::validateInternalColumn(self::PUT, $key, $op, $value)) {
                throw new PublicAlert('Your custom restful api validations caused the request to fail on column \'wp_options.\'.');
            }
        }
        unset($value);
