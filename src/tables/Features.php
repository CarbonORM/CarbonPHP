...
MYSQL;
    ...
    */
    ...
    {   
        ...
        if ($id === false) {
             $feature_entity_id = $id = self::fetchColumn('SELECT (REPLACE(UUID() COLLATE utf8_unicode_ci,"-",""))')[0];
        } else {
            $ref='carbon_features.feature_entity_id';
            $op = self::EQUAL;
            if (!self::validateInternalColumn(self::POST, $ref, $op, $feature_entity_id)) {
                return self::signalError('Your custom restful api validations caused the request to fail on column \'carbon_features.feature_entity_id\'.');
            }            
        }
        ...
        if (!self::validateInternalColumn(self::POST, $ref, $op, $feature_code)) {
            return self::signalError('Your custom restful api validations caused the request to fail on column \'carbon_features.feature_code\'.');
        }
        ...
        if (self::$commit && !Database::commit()) {
           return self::signalError('Failed to store commit transaction on table carbon_features');
        } 
    ...
    public static function Put(array &$returnUpdated, string $primary = null, array $argv = []) : bool
    {
        ...
        foreach ($argv as $key => &$value) {
            ...
            if (!self::validateInternalColumn(self::PUT, $key, $op, $value)) {
                return self::signalError('Your custom restful api validations caused the request to fail on column \'carbon_features.\'.');
            }
        }
        unset($value);
        ...
        if (array_key_exists('carbon_features.feature_code', $argv)) { 
            ...
            if (!self::validateInternalColumn(self::PUT, $ref, $op, $feature_code)) {
                return self::signalError('Your custom restful api validations caused the request to fail on column \'feature_code\'.');
            }
            $stmt->bindParam(':feature_code',$feature_code, PDO::PARAM_STR, 30);
        }
        if (array_key_exists('carbon_features.feature_creation_date', $argv)) { 
        ...
        $argv = array_combine(
            array_map(
                static fn($k) => str_replace('carbon_features.', '', $k),
                array_keys($argv)
            ),
            array_values($argv)
        );
    ...
    */
    public static function Delete(array &$remove, string $primary = null, array $argv = []) : bool
    {
        ...
        $sql =  /** @lang MySQLFragment */ 'DELETE FROM carbon_features ';
