...
use CarbonPHP\Abstracts\Restful\RestTemplates;
...
class RestBuilder extends Command
{
    // database
            ...
            switch ($argv[$i]) {
                ...
                case '-v':
                    if (isset($argv[++$i]) && strtolower($argv[$i]) === 'debug') {
                        echo "\tDebug mode is best when paired with the optional (-l or -f) flags. Use -help for more information.\n";
                        $debug = true;
                    } else {
                        --$i;
                    }
                ...
                case '-f':
                    if (empty($file = file_get_contents((string)$argv[++$i]))) {
                        echo 'Could not open file [ ' . $argv[$i] . " ] for input\n\n";
                        exit(1);
                    }
...
END;
        ...
        if (empty($targetDir)) {
            echo 'You must provide a target directory.' . PHP_EOL;
            $this->usage();
        } elseif (!is_dir($targetDir)) {
            echo 'Directory does not exist, attempting to create it.' . PHP_EOL;
            if (!mkdir($targetDir, 0755, true) && !is_dir($targetDir)) {
                echo 'The target directory appears invalid "' . $targetDir . '"' . PHP_EOL;
                exit(1);
            }
        }
        ...
        if (!is_dir($targetDirTraits)) {
            ...
            if (!mkdir($targetDirTraits, 0755, true) && !is_dir($targetDirTraits)) {
                echo 'The target directory appears invalid "' . $targetDirTraits . '"' . PHP_EOL;
                exit(1);
            }
        }

        if (empty($this->schema) || $this->schema === '') {
            echo 'You must specify the table schema!' . PHP_EOL;
            exit(1);
        }
        ...
        if (!file_exists(MySQL::$mysqldump)) {
            echo 'Could not load mysql dump file!' . PHP_EOL;
            exit(1);
        }

        if (empty(MySQL::$mysqldump = file_get_contents(MySQL::$mysqldump))) {
            echo 'Contents of the mysql dump file appears empty. Build Failed!';
            exit(1);
        }
            ...
            foreach ($linesInCreateTableStatement as $fullLineInCreateTableStatement) {
                ...
                switch ($wordsInLine[0]) {
                    case 'CREATE':
                        ...
                        if (file_exists($validation = $targetDir . $etn . '.php')) {
                            ...
                            if (false === preg_match_all(
                                ...
                                echo 'An unexpected regex error occurred during the namespace matching/cache';
                            ...
                            if (!class_exists($fullTableClassName)) {
                                echo "We failed to load the class methods for the table '$fullTableClassName'. It was located in $targetDir, but cannot be auto-loaded. Please add the location to Composers PSR-4.\n\n";
                                exit(1);
                            }
                            ...
                            if ($generatedClassCustomMethods === null) {
                                echo 'An unexpected error occurred when using get_class_methods.';
                                exit(1);
                            }
                    ...
                    case 'CONSTRAINT':
                        ...
                        $rest[$tableName]['CARBON_CARBONS_PRIMARY_KEY'] = ($rest[$tableName]['CARBON_CARBONS_PRIMARY_KEY'] === true)
                        ...
                        } elseif (!isset($rest[$references_table]['EXTERNAL_TABLE_CONSTRAINTS'])) {
                            $rest[$references_table]['EXTERNAL_TABLE_CONSTRAINTS'] = [];
                        }
                        ...
                        } elseif (!isset($rest[$references_table]['dependencies'])) {
                            $rest[$references_table]['dependencies'] = [];
                        }
                    ...
                    default:
                        ...
                        if ($wordsInLine[0][0] === '`') {
                            ...
                            if ($key !== false) {
                                ...
                                } elseif (!str_starts_with($default, '\'')) {
