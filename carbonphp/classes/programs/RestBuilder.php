...
use CarbonPHP\Traits\RestTemplates;
...
END;
                ...
                switch ($wordsInLine[0]) {
                    case 'CREATE':
                        ...
                        if (file_exists($validation = $targetDir . $etn . '.php')) {
                            ...
                            $columnNamespace = "use " . (str_ends_with($target_namespace, '\\') ? $target_namespace : $target_namespace . '\\') . "Tables\\Traits\\t{$etn};";

                            print $columnNamespace;
        ...
        $class = self::restTemplate();

        $typeClassTemplate = self::restTableClassTypeDefinition();

        $typeTraitTemplate = self::restTableTraitDefinition();
        ...
        foreach ($rest as $tableName => $parsed) {
            ...
            Files::mkdir($targetDir . 'classes' . DS);

            if (false === file_put_contents($targetDir . 'classes' . DS . $parsed['ucEachTableName'] . '_Columns.php', $mustache->render($typeClassTemplate, $parsed))) {

                ColorCode::colorCode('PHP internal file_put_contents failed while trying to store :: (' . $targetDir . 'classes' . DS . $parsed['ucEachTableName'] . '_Columns.php)', iColorCode::RED);

            }

            Files::mkdir($targetDir .  'traits' . DS );

            if (false === file_put_contents($targetDir . 'traits' . DS . 't' . $parsed['ucEachTableName'] . '.php', $mustache->render($typeTraitTemplate, $parsed))) {
