...
class ThrowableHandler
{
    ...
    public const string LOG_ARRAY = 'LOG_ARRAY';
    public const string HTML_ERROR_PAGE = 'HTML_ERROR_PAGE';
    public const string STORED_HTML_LOG_FILE_PATH = 'STORED_HTML_LOG_FILE_PATH';
    public const string STORAGE_LOCATION_KEY = 'THROWABLE STORED TO FILE';
    public const string TRACE = 'TRACE';
    public const string GLOBALS_JSON = '$GLOBALS[\'json\']';
    public const string INNODB_STATUS = 'INNODB_STATUS';
    public const string DEBUG_BACKTRACE = 'debug_backtrace()';

    // todo - defaultLocation this does nothing.
    public static ?string $defaultLocation = null;

    /**
     * @var bool determine if a generated error log should be shown on the browser.
    ...
    public static function checkCreateLogFile(string &$message, bool $fixFilePermissions = true): void
    {
        try {
            ...
            echo $e->getMessage().' (file://'.$e->getFile().':'.$e->getLine().')';
    ...
    public static function exitAndSendBasedOnRequested(array $json, ?string $html = null): never
    {
        ...
        if ($sendJson) {
            ...
            echo $html ?? self::generateBrowserReport($json, true);
        }
     ...
     * @return int
     */
    public static function start(ErrorConfiguration $configuration): int
    {
        // @link https://stackoverflow.com/questions/60000391/php-missing-function-arguments-in-exception-stack-trace
        ini_set('zend.exception_ignore_args', 0); // ensure stack traces include arguments

        ini_set('display_errors', $configuration->show);
         ...
         * @see https://www.php.net/manual/en/function.set-exception-handler.phpd
         */
        $error_handler = static function (int $errorLevel, string $errorString, string $errorFile, int $errorLine) {
            ...
            $fatalErrorKey = 'FATAL ERROR';

            $errorMap = [
                $fatalErrorKey => [
                    E_ALL, E_ERROR, E_RECOVERABLE_ERROR, E_PARSE, E_DEPRECATED, E_COMPILE_ERROR, E_USER_ERROR,
                ],
                'RECOVERABLE WARNING' => [
            ...
            foreach ($errorMap as $category => $errors) {
                if (in_array($errorLevel, $errors, true)) {
                    $browserOutput[$category] = array_search($errorLevel, $errors, true);
                    break;
                }
            }

            if (!isset($browserOutput)) {
                $browserOutput[$fatalErrorKey] = "Unknown error type ($errorLevel) with message ($errorString)";
                $fatalError                    = true;
            }

            $browserOutput[$fatalError ? 'ERROR MESSAGE' : 'WARNING MESSAGE'] = $errorString;
    ...
    public static function stop(bool $ignoreRedundantStops = false): void
    {
        if (false === $ignoreRedundantStops && self::$old_error_level === null) {
             ...
             * @see https://stackoverflow.com/questions/1095860/is-there-any-way-to-show-or-throw-a-php-warning
             **/
    ...
    public static function generateLog(?Throwable $e = null, ?bool $return = false, ?string $level = null, array &$log_array = [], string $color = iColorCode::RED): array
    {
        ...
        if (self::$storeReport === true) {
            ...
            } elseif (Database::$carbonDatabaseInitialized) {
                try {
        ...
        if (false === $return) {
            if (false === CarbonPHP::CLI) {    // we have alreay logged
                if (headers_sent()) {
                    ...
                    ColorCode::colorCode(
                        'HEADERS ALREADY SENT! Reporting to the browser can potentially be made more readable if you choose'
