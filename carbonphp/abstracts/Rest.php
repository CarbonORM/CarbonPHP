...
use CarbonPHP\Abstracts\Restful\RestLifeCycle;
use CarbonPHP\Abstracts\Tables\Carbons;
...
abstract class Rest extends RestLifeCycle
{
        ...
        do {
            try {
                ...
                if (null !== $primary && false === is_array($primary)) {
                    throw new PrivateAlert('Looks like your restful validations changed the primary value to an invalid state. The $primary field should be null or an array with the following syntax :: [ Table::EXAMPLE_COLUMN => "primary_key_string" ]  The value ('.json_encode($primary).') was instead received. ');
                }

                if (false === is_array($argv)) {
                    throw new PrivateAlert('Looks like your restful validations changed the $argv value to an invalid state. The $argv was not an array. Received :: ('.json_encode($argv).')');
                }
    ...
    protected static function updateReplace(array &$returnUpdated, array $argv = [], ?array $primary = null): bool
    {
        try {
            ...
            } elseif (array_key_exists(self::UPDATE, $argv)) {
            ...
            } elseif (false === self::$allowFullTableUpdates || !empty($where)) {
                if (empty($where)) {
                ...
                do {
                    ...
                    foreach (static::PDO_VALIDATION as $fullName => $info) {
                        ...
                        if ($fullName === static::PRIMARY
                            ...
                            } elseif (false === self::validateInternalColumn($fullName, $op, $iValue[$fullName])) {
                             ...
                             *
                             * @see https://stackoverflow.com/questions/28251144/inserting-and-selecting-uuids-as-binary16
                             * @see https://www.php.net/ChangeLog-8.php
                             *
    ...
    private static function verifyRowsAffected(
        \PDOStatement $stmt,
        string $method,
        string $sql,
        array|string|null $primary,
        ?array $argv,
    ): bool {
