...
abstract class RestQueryValidation extends RestAutoTargeting
{
    ...
    public static function disallowPublicAccess($request, ?string $calledFrom = null): void
    {
        if (self::$externalRestfulRequestsAPI && !CarbonPHP::$test) {
            ...
            throw new PrivateAlert('Rest request denied by the PHP_VALIDATION\'s in the tables ORM. Remove DISALLOW_PUBLIC_ACCESS ie (static fn() => self::disallowPublicAccess(self::class)) '.(null !== $calledFrom ? "from ($calledFrom) " : '').'to gain privileges. Method: ('.$_SERVER['REQUEST_METHOD'].') Uri: ('.$_SERVER['REQUEST_URI'].') Request: ('.json_encode($request, JSON_PRETTY_PRINT).')');
        }
    }
    ...
    public static function runRegexValidations($column, $value): ?string
    {
        ...
        foreach ($php_validation as $key => $validation) {
            ...
            if (!is_array($validation)) {
                if (is_callable($validation)) {
                    ...
                    } elseif (false === call_user_func_array($validation, [&$rest])) {
            ...
            } elseif (false === call_user_func_array([$class, $validationMethod], [&$rest, ...$validation])) {
