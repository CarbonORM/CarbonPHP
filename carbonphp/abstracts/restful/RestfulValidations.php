...
use CarbonPHP\Abstracts\DatabaseAnalyzer;
...
abstract class RestfulValidations extends DatabaseAnalyzer
{
    ...
    public static function putHistoryLog(): callable
    {
        ...
        return static function ($response) use (&$hasRun): void {
            ...
            if (empty(self::$historyLogId) || empty(self::$historyQuery)) {
                throw new PublicAlert('Failed to complete history log. ('.print_r(['$historyLogId' => self::$historyLogId, '$historyQuery' => self::$historyQuery], true).')');
            }
     ...
     * @return array
     */
    public static function getDefaultRestAccess(DatabaseHost $databaseConfiguration, string $selfClass, array $overrides = []): array
    {
        try {
            ...
            return [
                ...
                iRest::PUT => [
                    ...
                    iRest::PREPROCESS => [
                        ...
                        static function () use ($databaseConfiguration, &$prePutOptions) {
                            if (false === Rest::$externalRestfulRequestsAPI) {
                                return;
                            }

                            $prePutOptions = $databaseConfiguration->pdo_options;
                    ...
                    iRest::FINISH => [
                        ...
                        static function () use ($databaseConfiguration) {
                            if (false === Rest::$externalRestfulRequestsAPI) {
                                return;
                            }
                            $databaseConfiguration->commit(true);
                        },
                    ],
                ],
    ...
    public static function validateGeneratedExternalSqlRequest(string $sql, DatabaseHost $configuration): void
    {
        global $json;
        ...
        $shouldExit = $configuration->validateExternalRequests;
        ...
        $possibleSql = $configuration->validatedRestRequests;
