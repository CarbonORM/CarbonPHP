...
use CarbonPHP\Abstracts\Database;
use CarbonPHP\Abstracts\Error\PrivateAlert;
use CarbonPHP\Abstracts\Interfaces\iColorCode;
use CarbonPHP\Abstracts\Interfaces\iRest;
use CarbonPHP\Abstracts\Interfaces\iRestMultiplePrimaryKeys;
use CarbonPHP\Abstracts\Interfaces\iRestNoPrimaryKey;
use CarbonPHP\Abstracts\Interfaces\iRestSinglePrimaryKey;
use CarbonPHP\Abstracts\Tables\History_Logs;
use PDO;
...
abstract class RestQueryBuilder extends RestQueryValidation
{
    ...
    public static function handleSubSelectAggregate(array $stmt): string
    {
        ...
        if (false === self::$allowSubSelectQueries) {
            throw new PrivateAlert("Whoa, looks like a sub-select was encountered without `$tableName::\$allowSubSelectQueries` explicitly set to true. ".' This is a security precaution and is required to only be set to true when explicitly needed. You should'." consider doing this in the ($tableName::PHP_VALIDATION['PREPROCESS']) event cycle.");
        }
        ...
        if (true === self::is_assoc($stmt)) {   // todo - php 8.1 check when we gain support
            throw new PrivateAlert('The restful sub-select query has been detected to be an associative array. Associative array arrays can not (yet) be unpacked in PHP. Please remove the keys an allow php to manually assign them. The infringing set on table ('.static::class.') :: ('.json_encode($stmt).')');
        }
        ...
        if (null === $primaryField) {
            switch ($setLength) {
                ...
                default:
                    ...
                    throw new PrivateAlert('The restful sub-select set passed was not the correct length. The '."table ($tableName) has no primary keys so only three or four values are expected. Arguments must match the signature [ SELECT, $tableName, \$argv = [], \$as = '' ] for sub-select".' queries on tables which have no sql queries. The infringing set on table ('.static::class.') :: ('.json_encode($stmt).')');
            }
        } elseif (is_array($primaryField) || is_string($primaryField)) {
            ...
            switch ($setLength) {
                ...
                default:
                    ...
                    throw new PrivateAlert('The restful sub-select set passed was not the correct length. ('.count($stmt).') '.$error_context($tableName));
            }
        }
    ...
    private static function buildAggregate(array $stmt, bool $isSubSelect = false): string
    {
        ...
        if (false === in_array($aggregate, $allowedValues, true)) {
            throw new PrivateAlert('The attempted aggregate ('.$aggregate.') in query ( '.json_encode($stmt).') in the GET request must be one of the following: '.implode(', ', $allowedValues));
        }
...
TRIGGER;
    ...
    private static function buildQuerySelectValues(array $get, bool $isSubSelect): string
    {
        ...
        foreach ($get as $key => $column) {
            ...
            throw new PrivateAlert("CarbonPHP could not validate a column ($column) passed in the (SELECT) request which caused ".'the request to fail. It does not appear to be allowed based on the tables joined. Possible values include ('.json_encode(self::$compiled_valid_columns, JSON_PRETTY_PRINT).').');
        }
            ...
            foreach ($set as $column => $value) {
                ...
                switch ($count) {
                    ...
                    default:
                        ...
                        throw new PrivateAlert("A rest key column ($column) value (".json_encode($value).") was set to an array with ($count) ".'values. Boolean comparisons can use one of the following operators ('.implode('|', $supportedOperators).'). The same comparison can be made with an empty (aka default numeric) key and three array entries :: [  Column,  Operator, (Operand|Column2) ]. Both ways are made equally for conditions which might require multiple of the same key in the same array; as this would be invalid syntax in php.');
                }
            } // end foreach
        }
    ...
    protected static function addSingleConditionToWhereOrJoin($valueOne, string $operator, $valueTwo): string
    {
        ...
        if ($key_is_custom && $value_is_custom) {
            throw new PrivateAlert("Rest failed in as you have custom columns ($valueOne) &| ($valueTwo). This may mean you need to regenerate your rest tables, have misspellings in your request, have incorrect aggregation, or join conditions.".' Possible allowed columns are ('.implode(',', self::$compiled_valid_columns).').');
        }
