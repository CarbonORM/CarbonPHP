...
{
    public static function sendResource($file, $ext): never
    {
    ...
    public static function generateUpToDateMimeArray(): void
    {
        ...
        echo @sort($s) ? 'return array(<br />'.implode(',<br />', $s).'<br />);' : false;
    }
    ...
    public static function uploadFile(array $fileArray, string $location): ?string
    {
        try {
            ...
            if (!isset($fileArray['error'])) {
                throw new RuntimeException('Files could not be uploaded.');                 // changes to catch via error handler
            }
    ...
    public static function largeHttpGetRequestsToFile(string $url, string $toLocalFilePath, array &$responseHeaders = [], int $timeout = 3): void
    {
        ...
        try {
            ...
            if (CarbonPHP::$verbose) {
                ColorCode::colorCode(
                    "Detected in verbose mode, will not unlink file\nfile://$tmpPath",
                    iColorCode::YELLOW
                );
